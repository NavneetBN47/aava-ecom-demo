...
public Integer applyMinimumQuantityThreshold(Product product, Integer requestedQuantity) {
    if (product.getMinQuantity() != null && product.getMinQuantity() > 0) {
        return Math.max(requestedQuantity, product.getMinQuantity());
    }
...
Enhanced product validation with minimum quantity:
