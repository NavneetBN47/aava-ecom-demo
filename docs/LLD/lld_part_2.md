...
sequenceDiagram
    ...
    alt Product Not Found
        ...
        Note over CartService: Create/Update cart item with quantity 1
        ...
        Note over CartService: Update cart total
        CartService->>+CartRepository: save(cart)
        CartRepository->>+Database: UPDATE cart SET updated_at = ?, total_amount = ?
...
sequenceDiagram
    ...
    alt Cart Exists
        ...
        Database-->>-CartItemRepository: List<CartItem> with Product details
        CartItemRepository-->>-CartService: List<CartItem>
        
        Note over CartService: Calculate totals for all items
        Note over CartService: Populate cart with items and total
...
sequenceDiagram
    ...
    alt Item Not Found
        ...
        Note over CartService: Verify stock availability
        CartService->>+ProductRepository: findById(productId)
        ProductRepository->>+Database: SELECT * FROM products WHERE id = ?
        ...
        CartService->>+CartRepository: save(cart)
        CartRepository->>+Database: UPDATE cart SET total_amount = ?, updated_at = ?
        Database-->>-CartRepository: Cart
        CartRepository-->>-CartService: Cart
...
sequenceDiagram
    ...
    alt Item Not Found
        ...
        Note over CartService: Get cart ID from item
        ...
        Note over CartService: Update total amount
        CartService->>+CartRepository: save(cart)
        CartRepository->>+Database: UPDATE cart SET total_amount = ?, updated_at = ?
...
sequenceDiagram
    ...
    alt Cart Not Found or No Items
        ...
        Note over CartController: Add message: "Your cart is empty"
        Note over CartController: Add link: "Continue Shopping"
