...
sequenceDiagram
    ...
    Client->>+CartController: POST /api/cart/items (productId, quantity=1)
...
sequenceDiagram
    ...
    alt Cart Exists
        ...
        alt Cart is Empty
            CartService-->>CartController: Empty cart response with message
            CartController-->>Client: ResponseEntity with "Your cart is empty" + continue shopping link
        else Cart has Items
...
sequenceDiagram
    ...
    Client->>+CartController: PUT /api/cart/items/{itemId} (new quantity)
    ...
    alt CartItem Exists
        ...
        Note over CartService: Validate quantity <= stock_quantity
        Note over CartService: Calculate new subtotal = quantity × price
        Note over CartService: Update cartItem.quantity
        Note over CartService: Update cartItem.subtotal
...
sequenceDiagram
    ...
    alt CartItem Exists
        Note over CartService: Get cartId from cartItem
        ...
        Note over CartService: Recalculate cart total
        Note over CartService: Update cart.updatedAt
...
| POST | `/api/cart/items` | Add product to cart with quantity 1 | {productId, quantity} | CartItem |
| GET | `/api/cart` | View cart with all items, names, prices, quantities, subtotals, and total | None | ShoppingCart |
| PUT | `/api/cart/items/{itemId}` | Update cart item quantity with automatic subtotal and total recalculation | {quantity} | CartItem |
...
CREATE TABLE shopping_carts (
    ...
    CONSTRAINT uk_shopping_carts_user_id UNIQUE (user_id)
);
...
### 9.1 Automatic Recalculation Logic

Whenever a cart item quantity is updated, the system automatically:
1. Recalculates the item subtotal: `subtotal = quantity × price`
2. Recalculates the cart total: `total = sum of all item subtotals`
3. Updates the cart's `updated_at` timestamp
4. Persists all changes atomically within a transaction

This ensures data consistency and eliminates manual calculation errors.

### 9.2 Empty Cart Handling

When a user views their cart and it contains no items:
1. The system detects the empty state using `isCartEmpty(cartId)` method
2. Returns a structured response with:
   - Message: "Your cart is empty"
   - HTTP Status: 200 OK
   - Link to continue shopping: `/api/products`
3. Provides a seamless user experience for empty cart scenarios
