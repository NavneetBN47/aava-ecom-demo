...
sequenceDiagram
    ...
    CartController->>+CartService: addItemToCart(customerId, productId, isSubscription)
    ...
    Note over CartService: Check if product exists
    ...
    Note over CartService: Determine quantity based on:<br/>1. Minimum Procurement Threshold<br/>2. Subscription vs one-time buy<br/>Default to 1 if no threshold
    
    Note over CartService: Calculate subtotal = quantity × unit_price
...
sequenceDiagram
    ...
    CartController->>+CartService: getOrCreateCart(customerId)
    ...
    alt Cart Exists
        ...
        alt Cart Has Items
            ...
            CartController-->>Client: ResponseEntity with "Cart is empty" message (200)
        end
...
sequenceDiagram
    ...
    Note over CartService: Validate item exists
    ...
    Note over CartService: Validate inventory:<br/>quantity <= stock_quantity
    
    alt Inventory Available
        Note over CartService: Update quantity<br/>Recalculate subtotal = quantity × unit_price
...
| GET | `/api/cart` | View shopping cart | Query: customerId | ShoppingCart |
| POST | `/api/cart/items` | Add product to cart | CartItemRequest | CartItem |
| PUT | `/api/cart/items/{itemId}` | Update cart item quantity | { quantity: Integer } | CartItem |
| DELETE | `/api/cart/items/{itemId}` | Remove product from cart | None | None |
