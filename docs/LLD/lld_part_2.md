...
sequenceDiagram
    ...
    Client->>+CartController: POST /api/cart/items (customerId, productId, quantity, isSubscription)
    CartController->>+CartService: addItemToCart(customerId, productId, quantity, isSubscription)
    ...
    Note over CartService: Set default quantity = 1 if not provided
...
sequenceDiagram
    ...
    alt Cart Exists
        ...
        loop For each CartItem
            ...
            Database-->>-ProductRepository: Product
            ProductRepository-->>-CartService: Product
            Note over CartService: Enrich CartItem with product_name
        end
...
sequenceDiagram
    ...
    alt CartItem Exists
        ...
        Note over CartService: Validate inventory for new quantity
...
sequenceDiagram
    ...
    alt CartItem Exists
        ...
        CartService-->>CartController: void with updated cart total
...
| POST | `/api/cart/items` | Add product to cart | AddCartItemRequest | CartItem |
| GET | `/api/cart` | Get cart with all items | customerId (query param) | ShoppingCart |
| PUT | `/api/cart/items/{cartItemId}` | Update cart item quantity | UpdateQuantityRequest | CartItem |
...
- Support for subscription and one-time purchase models
...
- **Default Quantity:** When adding a product to cart, if quantity is not specified, default to 1
- **Minimum Procurement Threshold:** If a product has a `minimum_procurement_threshold` defined, automatically set the quantity to this threshold value when adding to cart
- **Subscription vs One-Time Buy:** 
  - For subscription purchases (`is_subscription = true`), quantity increments follow subscription rules
  - For one-time purchases (`is_subscription = false`), standard quantity rules apply
- **Inventory Validation:** Before adding items, validate that requested quantity does not exceed available `stock_quantity`

### 9.2 Cart Display Logic

- **Empty Cart Handling:** When cart has no items, return message "Your cart is empty" with navigation option to product catalog
- **Item Details:** Display product_name, unit_price, quantity, and subtotal for each cart item
- **Cart Total:** Calculate and display total cart value as sum of all item subtotals

### 9.3 Update Quantity Logic

- **Real-Time Calculation:** Recalculate subtotal and cart total immediately without page refresh
- **Inventory Validation:** Validate new quantity against available stock before updating
- **Error Handling:** Return error message when requested quantity exceeds available inventory

### 9.4 Remove Item Logic

- **Automatic Recalculation:** After removing an item, automatically recalculate cart total
- **Empty Cart Check:** If last item is removed, display empty cart message
