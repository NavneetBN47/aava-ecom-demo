...
sequenceDiagram
    ...
    Client->>+CartController: POST /api/cart/items (userId, productId, quantity=1)
    CartController->>+CartService: addItemToCart(userId, productId, 1)
    ...
    Note over CartService: Set unitPrice from product
    ...
    CartRepository->>Database: UPDATE shopping_carts SET updated_at = ?
...
sequenceDiagram
    ...
    CartController->>+CartService: getOrCreateCart(userId)
    ...
    alt Cart Found
        ...
        alt Cart Has Items
            Note over CartService: Calculate total = sum of all subtotals
            ...
            CartService-->>CartController: ShoppingCart (empty)
...
sequenceDiagram
    ...
    alt Item Found
        ...
        CartService-->>CartController: CartItem with updated subtotal
...
| GET | `/api/cart` | View cart with all items | Query: userId | ShoppingCart |
| POST | `/api/cart/items` | Add product to cart | CartItemRequest (userId, productId, quantity=1) | CartItem |
| PUT | `/api/cart/items/{itemId}` | Update item quantity | UpdateQuantityRequest (quantity) | CartItem |
...
The system automatically calculates and maintains cart totals:

- **Subtotal Calculation:** For each cart item, subtotal = quantity × unit_price
- **Cart Total Calculation:** Cart total = sum of all cart item subtotals
- **Automatic Recalculation:** Totals are recalculated automatically when:
...
When a cart has no items:
- API returns HTTP 200 with empty items list
- Response includes message: "Your cart is empty"
- Response includes navigation link to continue shopping
- Cart entity exists but items collection is empty

### 9.3 Cart Item Management Rules

- **Add to Cart:** Default quantity is 1 when adding a product
- **Duplicate Prevention:** Same product cannot be added twice to the same cart (enforced by unique constraint)
- **Quantity Updates:** Minimum quantity is 1, maximum can be validated against stock
- **Price Consistency:** Unit price is captured at time of adding to cart from product's current price
- **Cascade Deletion:** Removing a cart deletes all associated cart items automatically